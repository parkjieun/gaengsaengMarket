    ...
    > 
        ...
        >
            <v-toolbar-title color ="#3f696e" style="font-size:0.9rem" id="chatTitle">
                <div>
                    <v-avatar size="30"><v-img :src="patnerImg"/></v-avatar><span class="mx-3">{{patner.nickName}}</span>
                </div>
        ...
        <v-list dense id="chatBody" v-if="messages.length">
            ...
            <v-list-item two-line v-for="item in messages" :key="item.messageId">
                ...
                <v-avatar size="30" style="margin-right: 5px;" class="my-auto"><v-img :src="patnerImg"/></v-avatar>
                <v-list-item-content class="py-1">
                    <v-list-item-title class="receiveMsg my-auto caption" >{{item.content}}</v-list-item-title>
                ...
                <div v-else class="row justify-end">
                    <v-list-item-content class="py-1" style="width:auto; max-width:180px;">
                        <v-list-item-title class="sendMsg my-auto caption" >{{item.content}}</v-list-item-title>
                    ...
                    <v-avatar size="30" style="margin-right: 5px;" class="my-auto"><v-img :src="myProfileImg"/></v-avatar>
                </div>
            </v-list-item>
        </v-list>
...
export default {
    ...
    created() {
        ...
        this.myProfileImg = baseURL + "/static/image/account/" + this.$store.state.myProfile.profileImg
        console.log("ak")
